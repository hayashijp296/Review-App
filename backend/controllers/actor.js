...
exports.updateActor = async (req, res) => {
  // phai co actorID duoc get thong qua req.params thi moi co the get duoc actor thong qua method findById
  const { name, about, gender } = req.body;
  const { file } = req;
...
exports.getLatestActors = async (req, res) => {
  const result = await Actor.find().sort({ createAt: '-1' }).limit(12);
  const actors = result.map((actor) => formatActor(actor));
